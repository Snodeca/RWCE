...
local ImageRankDisplay = subclass(CompoundElement)

function ImageRankDisplay:init(window, returnFuncOrValue, numCharacters, fontName, passedDisplayOptions)
...
function ImageRankDisplay:update()
  local valueString
...
return {
  ImageRankDisplay = ImageRankDisplay
}